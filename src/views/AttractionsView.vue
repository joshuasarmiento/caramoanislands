...
<template>
  <div class="container py-12 mt-20">
    <div class="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
      <div>
        <h1 class="font-island text-[4.5rem] text-green">Explore Caramoan</h1>
      ...
      <router-link v-for="attr in filteredAndSortedAttractions" :key="attr.id"
        ...
        <div class="relative overflow-hidden">
          ...
          <div class="absolute top-4 right-4">
            <span
              class="bg-white/90 backdrop-blur px-3 py-1 rounded-full text-[10px] font-bold text-ocean uppercase shadow-sm">
              {{ attr.group }}
            </span>
          </div>
        </div>
        ...
        <div class="p-6">
          ...
          <h3 class="text-xl font-bold mb-3 text-ocean group-hover:text-green transition-colors">{{ attr.name }}
          </h3>
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="highlight in attr.highlights.slice(0, 2)" :key="highlight"
              class="text-[10px] bg-ocean/5 text-ocean/80 px-2 py-1 rounded">
              {{ highlight }}
            </span>
          </div>
        </div>
